            ...
            <div class="modal-body p-0">
                <div class="card bg-white shadow border-0">
                    <div class="card-header pb-2 border-0 position-relative"
                        style="height: 100px; background: {{ $settings->color_default }} @if (auth()->user()->cover != '') url('{{ Helper::getFile(config('path.cover') . auth()->user()->cover) }}') @endif no-repeat center center; background-size: cover;">
                    ...
                    <div class="card-body px-lg-5 py-lg-5 position-relative">
                        ...
                        <form method="post" action="{{ url('send/tip') }}" id="formSendTip">
                            ...
                            <input type="number" min="{{ $settings->min_tip_amount }}"
                                max="{{ $settings->max_tip_amount }}" required
                                data-min-tip="{{ $settings->min_tip_amount }}"
                                data-max-tip="{{ $settings->max_tip_amount }}" autocomplete="off" id="onlyNumber"
                                class="form-control mb-3 tipAmount" name="amount"
                                placeholder="{{ __('general.tip_amount') }} ({{ __('general.minimum') }} {{ Helper::priceWithoutFormat($settings->min_tip_amount) }})">
                            @csrf
                            ...
                            @if (!request()->route()->named('live'))
                                ...
                                @foreach ($paymentGatewaysSubscription as $payment)
                                    ...
                                    <div class="custom-control custom-radio mb-3">
                                        <input name="payment_gateway_tip" required value="{{ $payment->name }}"
                                            id="tip_radio{{ $payment->name }}"
                                            @if ($allPayments->count() == 1 && Helper::userWallet('balance') == 0) checked @endif
                                    ...
                                    @if ($payment->name == 'Stripe')
                                        <div id="stripeContainerTip"
                                            class="@if ($allPayments->count() != 1) display-none @endif">
                            ...
                            @if (($settings->disable_wallet == 'on' && Helper::userWallet('balance') != 0) || $settings->disable_wallet == 'off')
                                <div class="custom-control custom-radio mb-3">
                                    <input name="payment_gateway_tip" required
                                        @if (Helper::userWallet('balance') == 0) disabled @endif value="wallet" id="tip_radio0"
                                        class="custom-control-input" type="radio">
                            ...
                            <div class="text-center">
                                ...
                                <button type="submit" id="tipBtn" class="btn btn-primary mt-4 tipBtn"><i></i>
                                    {{ __('auth.send') }}</button>
                            </div>
