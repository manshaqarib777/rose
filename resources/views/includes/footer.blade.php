<!-- FOOTER -->
<div
    class="py-5 @auth d-none d-lg-block @endauth  bg-white @if (
        (auth()->check() && auth()->user()->dark_mode == 'off' && $settings->footer_background_color == '#ffffff') ||
            (auth()->guest() && $settings->footer_background_color == '#ffffff')) border-top @endif">
            ...
            <div class="col-md-3">
                ...
                @if (
                    $settings->facebook != '' ||
                        ...
                        $settings->threads != '')
                    <div class="w-100">
                        <span class="w-100">{{ trans('general.keep_connect_with_us') }}
                            {{ trans('general.follow_us_social') }}</span>
                        <ul class="list-inline list-social m-0">
                            @if ($settings->twitter != '')
                                <li class="list-inline-item"><a href="{{ $settings->twitter }}" target="_blank"
                                        class="ico-social"><i class="bi-twitter-x"></i></a></li>
                            @endif

                            @if ($settings->facebook != '')
                                <li class="list-inline-item"><a href="{{ $settings->facebook }}" target="_blank"
                                        class="ico-social"><i class="fab fa-facebook"></i></a></li>
                            @endif

                            @if ($settings->instagram != '')
                                <li class="list-inline-item"><a href="{{ $settings->instagram }}" target="_blank"
                                        class="ico-social"><i class="fab fa-instagram"></i></a></li>
                            @endif

                            @if ($settings->pinterest != '')
                                <li class="list-inline-item"><a href="{{ $settings->pinterest }}" target="_blank"
                                        class="ico-social"><i class="fab fa-pinterest"></i></a></li>
                            @endif

                            @if ($settings->youtube != '')
                                <li class="list-inline-item"><a href="{{ $settings->youtube }}" target="_blank"
                                        class="ico-social"><i class="fab fa-youtube"></i></a></li>
                            @endif

                            @if ($settings->github != '')
                                <li class="list-inline-item"><a href="{{ $settings->github }}" target="_blank"
                                        class="ico-social"><i class="fab fa-github"></i></a></li>
                            @endif

                            @if ($settings->tiktok != '')
                                <li class="list-inline-item"><a href="{{ $settings->tiktok }}" target="_blank"
                                        class="ico-social"><i class="bi-tiktok"></i></a></li>
                            @endif

                            @if ($settings->snapchat != '')
                                <li class="list-inline-item"><a href="{{ $settings->snapchat }}" target="_blank"
                                        class="ico-social"><i class="bi-snapchat"></i></a></li>
                            @endif

                            @if ($settings->telegram != '')
                                <li class="list-inline-item"><a href="{{ $settings->telegram }}" target="_blank"
                                        class="ico-social"><i class="bi-telegram"></i></a></li>
                            @endif

                            @if ($settings->reddit != '')
                                <li class="list-inline-item"><a href="{{ $settings->reddit }}" target="_blank"
                                        class="ico-social"><i class="bi-reddit"></i></a></li>
                            @endif

                            @if ($settings->linkedin != '')
                                <li class="list-inline-item"><a href="{{ $settings->linkedin }}" target="_blank"
                                        class="ico-social"><i class="bi-linkedin"></i></a></li>
                            @endif

                            @if ($settings->threads != '')
                                <li class="list-inline-item"><a href="{{ $settings->threads }}" target="_blank"
                                        class="ico-social"><i class="bi-threads"></i></a></li>
                            @endif
                        </ul>
                    </div>
                @endif
                        ...
                        <ul class="list-unstyled">
                            ...
                            @if ($categoriesCount > 6)
                                <li><a class="link-footer btn-arrow"
                                        href="{{ url('creators') }}">{{ trans('general.explore') }}</a></li>
                            @endif
                        </ul>
                    </div>
                @endif
                ...
                <ul class="list-unstyled">
                    @guest
                        <li><a class="link-footer"
                                href="{{ $settings->home_style == 0 ? url('login') : url('/') }}">{{ trans('auth.login') }}</a>
                        </li>
                        <li>
                            @if ($settings->registration_active == '1')
                        <li><a class="link-footer"
                                href="{{ $settings->home_style == 0 ? url('signup') : url('/') }}">{{ trans('auth.sign_up') }}</a>
                        </li>
                        <li>
                            @endif
                        @else
                        <li><a class="link-footer url-user"
                                href="{{ url(auth()->User()->username) }}">{{ auth()->user()->verified_id == 'yes' ? trans('general.my_page') : trans('users.my_profile') }}</a>
                        </li>
                        <li>
                        <li><a class="link-footer"
                                href="{{ url('settings/page') }}">{{ auth()->user()->verified_id == 'yes' ? trans('general.edit_my_page') : trans('users.edit_profile') }}</a>
                        </li>
                        <li>
                        <li><a class="link-footer"
                                href="{{ url('my/subscriptions') }}">{{ trans('users.my_subscriptions') }}</a></li>
                        <li>
                        <li><a class="link-footer" href="{{ url('logout') }}">{{ trans('users.logout') }}</a></li>
                        <li>
                        @endguest
                            ...
                            <div class="dropdown-menu">
                                @foreach ($languages as $language)
                                    <a @if ($language->abbreviation != config('app.locale')) href="{{ url('change/lang', $language->abbreviation) }}" @endif
                                        class="dropdown-item mb-1 dropdown-lang @if ($language->abbreviation == config('app.locale')) active text-white @endif">
                                        @if ($language->abbreviation == config('app.locale'))
        ...
        <div class="row">
            ...
            <div class="col-md-12 copyright @auth d-none d-lg-block @endauth">
                ...
                @if ($settings->show_address_company_footer)
                    <small class="ml-2">
                        {{ $settings->company }} - {{ __('general.address') }}: {{ $settings->address }}
