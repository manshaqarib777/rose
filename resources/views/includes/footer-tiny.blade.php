    ...
    <div class="card-body p-0">
        <small class="text-muted">
            ...
            @if ($settings->show_address_company_footer)
                {{ $settings->company }} - {{ __('general.address') }}: {{ $settings->address }} {{ $settings->city }}
                {{ $settings->country }}
            @endif
        </small>
        ...
        <ul class="list-inline mb-0 small">
            ...
            @if (!$settings->disable_contact)
                <li class="list-inline-item"><a class="link-footer footer-tiny"
                        href="{{ url('contact') }}">{{ trans('general.contact') }}</a></li>
            @endif

            @if ($blogsCount != 0)
                <li class="list-inline-item"><a class="link-footer footer-tiny"
                        href="{{ url('blog') }}">{{ trans('general.blog') }}</a></li>
            @endif
                            ...
                            <div class="dropdown-menu">
                                @foreach ($languages as $language)
                                    <a @if ($language->abbreviation != config('app.locale')) href="{{ url('change/lang', $language->abbreviation) }}" @endif
                                        class="dropdown-item mb-1 @if ($language->abbreviation == config('app.locale')) active text-white @endif">
                                        @if ($language->abbreviation == config('app.locale'))
