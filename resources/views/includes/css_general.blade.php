...
@elseif(auth()->check() && auth()->user()->dark_mode == 'off')
...
@auth
    <link href="{{ asset('public/js/fileuploader/font/font-fileuploader.css') }}" media="all" rel="stylesheet"
        type="text/css" />
    <link href="{{ asset('public/js/fileuploader/jquery.fileuploader.min.css') }}" media="all" rel="stylesheet"
        type="text/css" />
    <link href="{{ asset('public/js/fileuploader/jquery.fileuploader-theme-thumbnails.css') }}" media="all"
        rel="stylesheet" type="text/css" />
    <link href="{{ asset('public/js/fileuploader/jquery.fileuploader-theme-dragdrop.css') }}" media="all"
        rel="stylesheet" type="text/css" />
    ...
    @if (request()->path() == '/' && $settings->story_status)
        <link href="{{ asset('public/js/story/zuck.min.css') }}?v={{ $settings->version }}" rel="stylesheet"
            type="text/css" />
        <link href="{{ asset('public/js/story/snapssenger.css') }}?v={{ $settings->version }}" rel="stylesheet"
            type="text/css" />
    @endif

    @if (
        (request()->path() == '/' && $settings->story_status && $fonts) ||
            (request()->is('create/story/text') && $settings->story_status && $fonts))
...
<script type="text/javascript">
    ...
    var ok = "{{ __('users.ok') }}";
    var user_count_carousel =
        @if (auth()->guest() && request()->path() == '/' && config('settings.home_style') == 0)
            {{ $users->count() }}
        @else
            0
        @endif ;
    ...
    var sortBy =
        "{{ in_array(request()->get('sort'), ['oldest', 'unlockable', 'free']) ? trim(request()->get('sort')) : false }}";
    ...
    var formats_available_verification =
        "{{ __('general.formats_available_verification_form_w9', ['formats' => 'JPG, PNG, GIF']) }}";
    ...
    var max_size_id_lang =
        "{{ __('general.max_size_id') . ' ' . Helper::formatBytes($settings->file_size_allowed_verify_account * 1024) }}";
    ...
    var stripeKey =
        "{{ PaymentGateways::where('id', 2)->where('enabled', '1')->whereSubscription('yes')->first() ? env('STRIPE_KEY') : false }}";
    var stripeKeyWallet =
        "{{ PaymentGateways::where('id', 2)->where('enabled', '1')->first() ? env('STRIPE_KEY') : false }}";
    ...
    var formats_available_verification_form_w9 =
        "{{ __('general.formats_available_verification_form_w9', ['formats' => 'PDF']) }}";
    ...
    @else
        var extensionsPostMessage = ['png', 'jpeg', 'jpg', 'gif', 'ief', 'video/mp4', 'video/quicktime', 'video/3gpp',
            'video/mpeg', 'video/x-matroska', 'video/x-ms-wmv', 'video/vnd.avi', 'video/avi', 'video/x-flv',
            'audio/x-matroska', 'audio/mpeg'
        ];
        var extensionsStories = ['png', 'jpeg', 'jpg', 'gif', 'ief', 'video/mp4', 'video/quicktime', 'video/3gpp',
            'video/mpeg', 'video/x-matroska', 'video/x-ms-wmv', 'video/vnd.avi', 'video/avi', 'video/x-flv'
        ];
    @endif
    var errorStoryMaxVideosLength =
        "{{ __('general.error_story_max_videos_length', ['length' => $settings->story_max_videos_length]) }}";
...
<style type="text/css">
    ...
    @if (auth()->check() || auth()->guest())
        ...
        .font-montserrat a:hover {
            color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #FFF @endif !important;
        }
        ...
        .search-bar:focus {
            ...
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #474747 @endif !important;
        }

        .text-notify {
            color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #8898aa @endif !important;
        }
        ...
        .dropdown-item.balance:hover {
            background: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222 @endif !important;
            color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #ffffff @endif !important;
        }
        ...
        .btn-google:focus {
            ...
            border-color: #ccc;
            color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #fff @endif;
        }

        .line-replies {
            border-color:  @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #fff @endif !important;
        }
        ...
        .input-group-text {
            border-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222 @endif;
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #303030 @endif;
        }

        .datepicker.dropdown-menu {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #303030 @endif !important;
        }

        .datepicker-dropdown.datepicker-orient-bottom:after {
            border-top: 6px solid  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #303030 @endif !important;
        }

        .datepicker-dropdown:after {
            border-bottom: 6px solid  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #303030 @endif !important;
        }

        .form-control{
            border-width: 1px;
            border-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #e9ecf1  @else #222 @endif;
        }

        .form-control:focus,
        .custom-select:focus {
            border-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222 @endif !important;

        }

        .custom-select {
            background:  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #303030 @endif url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' width='4' height='5' viewBox='0 0 4 5'%3e%3cpath fill='%23a5a5a5' d='M2 0L0 2h4zm0 5L0 3h4z'/%3e%3c/svg%3e") no-repeat right .75rem center/8px 10px;
            color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #fff @endif;
        }
        ...
        .sweet-alert {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #2f2f2f @endif;
        }

        .content-locked {
            background:  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #444444 @endif ;
        }

        @media (max-width: 991px) {
            .navbar .navbar-collapse {
                background:  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222 @endif;
            }
            ...
            .navbar-collapse .navbar-toggler span {
                background: @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #fff @endif;
            }
        }
        ...
        .btn-post:hover,
        .icons-live:hover {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222222 @endif !important;
        }
        ...
        .unread-chat {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #444 @endif !important;
        }
        ...
        @keyframes animate {
            ...
            to {
                background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #383838 @endif;
                transition: all 0.3s ease-out;
            }
        }

        .item-loading::before {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #6b6b6b @endif;
            content: ' ';
            display: block;
            height: 100%;
            position: absolute;
        ...
        .plyr--audio .plyr__controls {
            background: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #212121 @endif;
            color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #000  @else #fff @endif;
        }
        ...
        .sweet-alert .sa-icon.sa-success::before {
            background: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #2f2f2f @endif;
        }

        .page-item.disabled .page-link,
        .page-link {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222222 @endif;
        }
        ...
        .StripeElement {
            background-color:  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222222 @endif;
            border: 1px solid  @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222222 @endif ;
        }

        .StripeElement--focus {
            border-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #525252 @endif;
        }

        .bg-autocomplete {
            background-color: @if(auth()->check() && auth()->user()->dark_mode == 'off') #fff  @else #222 @endif;
        }
    ...
    .icon-navbar {
        ...
        @if ((auth()->check() && auth()->user()->dark_mode == 'on') || auth()->guest())
            color: #FFF !important;
        @endif
    }
    ...
    {{ $settings->button_style == 'rounded' ? '.btn, .sa-button-container button {border-radius: 50rem!important;}' : null }}
     @if ((auth()->check() && auth()->user()->dark_mode == 'off'))
        ...
        @media (max-width: 991px) {
            ...
            .navbar-collapse .navbar-toggler span {
                background-color: {{ $settings->navbar_text_color }} !important;
            }

            .dropdown-divider {
                border-top-color: {{ $settings->navbar_background_color }} !important;
            }
        }

        .footer_background_color {
            background-color: {{ $settings->footer_background_color }} !important;
        }
    ...
    @if ($settings->color_default != '')
        :root {
            ...
            --color-media-wrapper: @if (auth()->check() && auth()->user()->dark_mode == 'off')
                #f1f1f1
            @else
                #454545
            @endif
            ...
            --color-pulse-media-wrapper: @if (auth()->check() && auth()->user()->dark_mode == 'off')
                #f8f8f8
            @else
                #373737
            @endif
        ...
        .nav-profile li.active {
            border-bottom: 3px solid {{ auth()->check() && auth()->user()->dark_mode == 'on' ? '#fff' : '#fff' }} !important;
        }
        ...
        .button-white-sm {
            ...
            color: @if (auth()->check() && auth()->user()->dark_mode == 'on')
        ...
        .select2-container--default .select2-selection--multiple {
            ...
            background-color: @if (auth()->check() && auth()->user()->dark_mode == 'on')
                #303030 !important
            @elseif (auth()->check() && auth()->user()->dark_mode == 'off')
                #fff !important
            @else
            ...
            border-color: @if (auth()->check() && auth()->user()->dark_mode == 'on')
                #222 !important
            @elseif (auth()->check() && auth()->user()->dark_mode == 'on')
                #cad1d7 !important
            @else
                #222 !important    
            @endif
        ...
        @auth .icon-dashboard {
                ...
                background-color: @if (auth()->user()->dark_mode == 'on')
                    #414141 !important;
                @else
                    {{ $settings->color_default }}2b;
                @endif
                border-radius: 35%;

                color: @if (auth()->user()->dark_mode == 'on')
                    #FFF !important;
                @else
                    {{ $settings->color_default }} !important;
                @endif
                ;
            }
        ...
        @endguest
            ...
            @auth .preview-shop .fileuploader {
                    display: block;

                    background: @if (auth()->user()->dark_mode == 'on')
                        #414141 !important;
                    @else
                        #fafbfd !important;
                    @endif
                ...
                .ui-widget-content .ui-state-active {
                    ...
                    display: block;
                    border: 0 !important;
                    background: @if(auth()->check() && auth()->user()->dark_mode == 'on') #222 @elseif(auth()->check() && auth()->user()->dark_mode == 'off') #efefef @else #222 @endif;
                    color: #333;
                    text-decoration: none;
                    margin: 0;
                    border-radius: 6px;
                ...
                .icon--dashboard {
                    color: rgb(0 0 0 / @if(auth()->check() && auth()->user()->dark_mode == 'on') 5% @elseif(auth()->check() && auth()->user()->dark_mode == 'off') 2% @else 5% @endif) !important;
