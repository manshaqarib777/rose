<header>
    <nav style="border-bottom: 1px solid var(--secondary);"
        class="navbar navbar-expand-lg navbar-inverse p-nav @if (auth()->guest() && request()->path() == '/') scroll @else p-3 @if (request()->is('live/*')) d-none @endif  @if (request()->is('messages/*')) d-none d-lg-block shadow-sm @elseif(request()->is('messages')) shadow-sm @else shadow-none @endif {{ auth()->check() && auth()->user()->dark_mode == 'on' ? 'bg-white' : 'bg-white' }} link-scroll @endif">
        <div class="container-fluid d-flex position-relative">
            ...
            <a class="navbar-brand" href="{{ url('/') }}">
                @if (auth()->check() && auth()->user()->dark_mode == 'on')
                    <img src="{{ url('public/img', $settings->logo) }}" data-logo="{{ $settings->logo }}"
                        data-logo-2="{{ $settings->logo_2 }}" alt="{{ $settings->title }}"
                        class="logo align-bottom max-w-100" />
                @else
                    <img src="{{ url('public/img', auth()->guest() && request()->path() == '/' ? $settings->logo : $settings->logo_2) }}"
            ...
            @guest
                <button class="navbar-toggler @if (auth()->guest() && request()->path() == '/')  @endif" type="button"
                ...
                <ul class="navbar-nav ml-auto">
                    ...
                    @endif
						...
						<li class="nav-item dropdown d-lg-block">
                            <a href="#" data-toggle="modal" @guest data-target="#loginFormModal" @else data-target="#liveStreamingForm" @endguest class="nav-link px-2" title="{{ __('general.create_live_stream') }}">
                                <i class="feather icon-radio icon-navbar"></i>
                                <span class="d-lg-none align-middle ml-1">{{ __('general.create_live_stream') }}</span>
                            </a>
                        </li>
						...
						<li class="nav-item dropdown d-lg-block">
                            <a href="@auth {{ url(auth()->User()->username) }} @endauth" @guest data-toggle="modal" data-target="#loginFormModal" @endguest class="nav-link px-2" title="{{ __('auth.login') }}">
                                <i class="feather icon-user icon-navbar"></i>
                                <span class="d-lg-none align-middle ml-1">{{ __('auth.login') }}</span>
                            </a>
                        </li>

						{{-- <li class="nav-item dropdown d-lg-block d-none">
                            <a href="#" data-toggle="modal" data-target="#postFormModal" class="nav-link px-2" title="{{ __('general.my_posts') }}">
                                <i class="feather icon-plus icon-navbar"></i>
                                <span class="d-lg-none align-middle ml-1">{{ __('general.my_posts') }}</span>
                            </a>
                        </li> --}}
						...
						<li class="nav-item dropdown d-lg-block">
                            <a href="#" data-toggle="modal" @guest data-target="#loginFormModal" @else data-target="#postFormModal" @endguest class="nav-link px-2" title="{{ __('general.my_posts') }}">
                                <i class="feather icon-plus icon-navbar"></i>
                                <span class="d-lg-none align-middle ml-1">{{ __('general.my_posts') }}</span>
                            </a>
                        </li>
						...
						<li class="nav-item dropdown d-lg-block">
                            <a class="nav-link pl-2 " href="#" data-toggle="modal" data-target="#loginFormModal">
                                <img src="https://cdn-icons-png.flaticon.com/128/4128/4128176.png"
						...
						<li class="nav-item dropdown d-lg-block">
                            <a class="nav-link" href="#" id="nav-inner-success_dropdown_1" role="button"
                                ...
                                <img src="@if(auth()->user()->avatar == null) https://cdn-icons-png.flaticon.com/128/4128/4128176.png @else {{ Helper::getFile(config('path.avatar') . auth()->user()->avatar) }} @endif"
                                    alt="User" class="rounded-circle avatarUser mr-1" width="28" height="28">
