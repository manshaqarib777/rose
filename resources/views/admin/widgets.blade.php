...
@section('content')
    <h5 class="mb-4 fw-light">
        ...
        <span class="text-muted">{{ __('general.widgets') }} ({{ $totalWidgets }})</span>

        <a href="{{ url('panel/admin/widgets/add') }}" class="btn btn-sm btn-dark float-lg-end mt-1 mt-lg-0">
                                    ...
                                    @if ($totalWidgets != 0)
                                        ...
                                        @foreach ($widgets as $widget)
                                            <tr>
                                                ...
                                                <td>{{ $widget->title }}</td>
                                                <td>{{ $widget->type }}</td>
                                                <td>{{ $widget->content->name ?? null }}</td>
                                               
                                                <td>
                                                    <a href="{{ url('panel/admin/widgets/edit/') . '/' . $widget->id }}"
                                                        class="btn btn-success rounded-pill btn-sm me-2">
                                                        <i class="bi-pencil"></i>
                                                    </a>

                                                    <form method="POST"
                                                        action="{{ url('panel/admin/widgets/delete', $widget->id) }}"
                                                        accept-charset="UTF-8" class="d-inline-block align-top">
                                                        @csrf
                                                        <button class="btn btn-danger rounded-pill btn-sm actionDelete"
                                                            type="button"><i class="bi-trash-fill"></i></button>
                                                    </form>
