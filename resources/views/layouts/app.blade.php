...
<main @if (request()->is('messages/*') || request()->is('live/*')) class="h-100" @endif role="main">
    ...
    {{-- @if ((auth()->guest() && !request()->route()->named('profile') && !request()->is(['creators', 'category/*', 'creators/*'])) || (auth()->check() && !request()->route()->named('profile') && !request()->is(['my/bookmarks', 'my/likes', 'my/purchases', 'explore', 'messages', 'messages/*', 'creators', 'category/*', 'creators/*', 'live/*'])))

        @if ((auth()->guest() && request()->path() == '/' && $settings->home_style == 0) || (auth()->guest() && request()->path() != '/' && $settings->home_style == 0) || (auth()->guest() && request()->path() != '/' && $settings->home_style == 1) || auth()->check())
    ...
    @if (!request()->is(['messages', 'messages/*', 'live/*']))
        @include('includes.footer')
    @endif
