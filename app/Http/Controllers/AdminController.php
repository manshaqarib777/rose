...
class AdminController extends Controller
{
	...
	{
		$categ = Categories::orderBy('id');
		$parent = null;
		if($request->has('parent_id')){
			$categ->where('parent_id', $request->parent_id);
			$parent = Categories::find($request->parent_id);
		}else{
			$categ->where('parent_id', null);
		}

		$categories = $categ->get();
		$totalCategories = count($categories);
		

		return view('admin.categories', compact('categories', 'totalCategories','parent'));
	}
	...
	public function storeCategories(Request $request)
	{
		...
		$sql->parent_id   = $request->parent_id > 0 ? $request->parent_id : null;
	...
	public function updateCategories(Request $request)
	{
		...
		$categories->parent_id   = $request->parent_id > 0 ? $request->parent_id : null;
	...
	{
		$widgets      = Widget::orderBy('serial_no')->get();
		$totalWidgets = count($widgets);

		return view('admin.widgets', compact('widgets', 'totalWidgets'));
	}

	public function addWidgets()
	{
		return view('admin.add-widgets');
	}

	public function storeWidgets(Request $request)
	{
		...
		$sql              = new Widget();
		$sql->title        = $request->title;
		$sql->content_id   = $request->content_id > 0 ? $request->content_id : null;
		$sql->type        = $request->type;
		$sql->serial_no    = $request->serial_no;
		$sql->save();

		\Session::flash('success_message', __('admin.success_add'));

		return redirect('panel/admin/widgets');
	}
	...
	public function editWidgets($id)
	{
		...
		return view('admin.edit-widgets')->with('widgets', $widgets);
	}

	public function updateWidgets(Request $request)
	{
		$widgets        = Widget::find($request->id);
		...
		$widgets->content_id   = $request->content_id > 0 ? $request->content_id : null;
		$widgets->type   = $request->type;
		$widgets->serial_no    = $request->serial_no;
		$widgets->save();
